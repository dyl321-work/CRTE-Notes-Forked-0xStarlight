...
##### AD Module
```powershell
Get-ADObject -SearchBase "CN=System,$((Get-ADDomain).DistinguishedName)" -LDAPFilter "(objectClass=domainDNS)"
```
...
Get-ADUser student1 -Properties *
```

## Get specific properties for users in the current domain
##### PowerView
```powershell
Get-UserProperty
Get-UserProperty -Properties pwdlastset,logoncount,badpwdcount
...
##### AD Module
```powershell
Get-ADUser -Filter * -Properties pwdlastset | Select Name,pwdlastset
```
